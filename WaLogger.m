...
- (instancetype)discovery_init {
    ...
    if ([className containsString:@"WACall"] || [className containsString:@"CallManager"]) {
        log_debug(@"INSTANCE CREATED: %@", className);
    }
...
- (void)hook_generic_setter:(id)arg1 {
    log_debug(@"[%@] SETTER CALLED with: %@", NSStringFromClass([self class]), arg1);
...
static void init_discovery() {
    ...
    NSArray *classes = @[@"WhatsApp.WACall", @"WACall", @"_TtC8WhatsApp6WACall", @"WhatsApp.WAGroupCall"];
    for (NSString *name in classes) {
        Class cls = NSClassFromString(name);
        if (cls) {
            log_debug(@"Found candidate: %@. Hooking setters...", name);
